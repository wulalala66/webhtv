...
public class ConfigDialog extends BaseAlertDialog {
    ...
    protected void initView() {
        ...
        managed = Config.isManaged(type, ori = config.getUrl());
        binding.url.setText(managed ? "" : ori);
        binding.url.setSelection(managed || TextUtils.isEmpty(ori) ? 0 : ori.length());
        if (managed) binding.url.setHint(R.string.setting_vod);
    }
    ...
    private Config saveConfig(String url, String name) {
        ...
        if (url.isEmpty()) {
            if (!edit) return null;
            if (managed) return Config.find(ori, type).name(name).update();
