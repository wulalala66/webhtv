...
public class PlayerManager implements ParseCallback {
    ...
    private void setDanmakus(List<Danmaku> items) {
        ...
        setDanmaku(preferred);
    }

    private Danmaku getPreferredDanmaku(List<Danmaku> items) {
        if (items == null || items.isEmpty()) return Danmaku.empty();
        for (Danmaku item : items) if (item != null && item.isSelected() && !item.isEmpty()) return item;
        for (Danmaku item : items) if (item != null && !item.isEmpty()) return item;
    ...
    private void setDanmaku(Danmaku item, boolean force) {
        ...
        String apiUrl = DanmakuSetting.getValidApiUrl();
        if (TextUtils.isEmpty(apiUrl)) apiUrl = DanmakuSetting.getEffectiveApiUrl();
        if (TextUtils.isEmpty(apiUrl)) apiUrl = normalizeBase(BuildConfig.DANMAKU_PIZAZZ_BASE);
        if (TextUtils.isEmpty(apiUrl)) apiUrl = normalizeBase(BuildConfig.DANMAKU_UZDM_BASE);
        String url = DanmakuUrlPolicy.normalize(apiUrl, item.getRealUrl());
        ...
        if (!force && TextUtils.equals(currentDanmakuUrl, url) && danmakuController != null) {
        ...
        if (spec != null) spec.setDanmaku(item);
        if (force && danmakuController != null && currentDanmakuUrl != null) danmakuController.clearItems();
        currentDanmakuUrl = url;
        currentDanmakuKey = key;
        if (danmakuController == null) {
            if (SpiderDebug.isEnabled()) SpiderDebug.log("danmaku", "defer until controller ready name=%s %s", item.getName(), DanmakuUrlPolicy.logSummary(url));
            return;
        }
        if (sourceType.isLive()) {
            danmakuController.clearItems();
