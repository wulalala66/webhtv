...
public class Updater implements Download.Callback, UpdateListener {
    ...
    private Update getUpdate(String channel) {
        // 本 fork 仅使用自己的 GitHub Releases；保留 cnb 变量名以兼容后续 fallback 逻辑
        Update github = Update.CHANNEL_BETA.equals(channel) ? getGithubBetaUpdate(channel) : getGithubStableUpdate(channel);
        Update cnb = readUpdate(channel, Github.getCnbAsset(getManifestName(channel)), SOURCE_GITHUB);
        Update update = newer(github, cnb);
        attachDownloadFallback(update, github, cnb);
        return update;
    }
